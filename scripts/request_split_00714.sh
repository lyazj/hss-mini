...
DIROUT=root://eosuser.cern.ch//eos/user/l/legao/hss/samples/MiniAOD2/WJetsToQQ_HT-600to800_TuneCP5_13TeV-madgraphMLM-pythia8/HIG-RunIISummer20UL18MiniAODv2-00714-wcb
LOGDIR=log
mkdir -p "${LOGDIR}"
...
CONTENT="${CONTENT/__TO_BE_REPLACED_LOGDIR/${LOGDIR}}"
CONTENT="${CONTENT/__TO_BE_REPLACED_LOGDIR/${LOGDIR}}"
echo "${CONTENT}" > run_split_00714.jdl

for FILE in $(cat HIG-RunIISummer20UL18MiniAODv2-00714.txt); do
    FILEIN="${FILE}"
    FILEOUT="${DIROUT}/$(basename "${FILE/MiniAODv2/FilteredMiniAODv2}")"
    echo "${FILEIN}, ${FILEOUT}"
done | tee -a run_split_00714.jdl
echo ')' >> run_split_00714.jdl
